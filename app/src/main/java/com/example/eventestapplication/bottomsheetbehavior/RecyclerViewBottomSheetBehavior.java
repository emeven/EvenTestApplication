...
public class RecyclerViewBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    @Nullable
    MyViewDragHelper viewDragHelper;
    ...
    public boolean onLayoutChild(
        ...
        if (viewDragHelper == null) {
            viewDragHelper = MyViewDragHelper.create(parent, dragCallback);
        }
    ...
    public boolean onInterceptTouchEvent(
        ...
        View scroll = nestedScrollingChildRef != null ? nestedScrollingChildRef.get() : null;
        //Log.d("!!!!!!!", String.valueOf(isScrollViewOnTopMap.get(currentPagePosition)));
        return action == MotionEvent.ACTION_MOVE
                //&& (isScrollViewOnTopMap.get(currentPagePosition) == true)
                && scroll != null
                && !ignoreEvents
    ...
    public boolean onTouchEvent(
        ...
        if (action == ACTION_DOWN
                && !(slideOffset >=-1 && slideOffset <-0.95)
                && !(slideOffset >=-0.45 && slideOffset <-0.5)
                && !(slideOffset >=-0.05 && slideOffset <0.1)) {
        ...
        // ACTION_CANCEL   STATE_COLLAPSED
        if (action == MotionEvent.ACTION_UP || action == ACTION_CANCEL) {
            if (slideOffset < -0.5f)
    ...
    public void onStopNestedScroll(
        ...
        startSettlingAnimation(child, targetState, top, false, true, 0);
        nestedScrolled = false;
    }
    ...
    private void reset() {
        activePointerId = MyViewDragHelper.INVALID_POINTER;
        if (velocityTracker != null) {
    ...
    void settleToState(@NonNull View child, int state) {
        ...
        //当状态从隐藏更改为收起时，不需要最大时长限制
        boolean needMaxLimit = true;
        if (getState() == STATE_HIDDEN || slideOffset < -0.5f) {
            if (state == STATE_COLLAPSED)
                needMaxLimit = false;
        }
        Log.d("!!!!!!!!!!!!", String.valueOf(needMaxLimit));
        //改速度请调节下面得传参 newSettleDuration 最好以256为单位递增，越大越慢
        startSettlingAnimation(child, state, top, false, needMaxLimit, 1024);
    }

    void startSettlingAnimation(View child, int state, int top, boolean settleFromViewDragHelper, Boolean needMaxLimit, int newSettleDuration) {
        boolean startedSettling =
                settleFromViewDragHelper
                        ? viewDragHelper.settleCapturedViewAt(child.getLeft(), top)
                        : viewDragHelper.smoothSlideViewTo(child, child.getLeft(), top, needMaxLimit, newSettleDuration);
        if (startedSettling) {
    ...
    private final MyViewDragHelper.Callback dragCallback =
            new MyViewDragHelper.Callback() {
                ...
                public void onViewDragStateChanged(int state) {
                    if (state == MyViewDragHelper.STATE_DRAGGING && draggable) {
                        setStateInternal(STATE_DRAGGING);
                    }
                }
                ...
                public void onViewReleased(@NonNull View releasedChild, float xvel, float yvel) {
                    ...
                    startSettlingAnimation(releasedChild, targetState, top, true, true, 0);
                }

...
public class MyViewDragHelper {
    ...
    private final MyViewDragHelper.Callback mCallback;
     ...
     */
    public static MyViewDragHelper create(@NonNull ViewGroup forParent, @NonNull MyViewDragHelper.Callback cb) {
        return new MyViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static MyViewDragHelper create(@NonNull ViewGroup forParent, float sensitivity,
                                          @NonNull MyViewDragHelper.Callback cb) {
        final MyViewDragHelper helper = create(forParent, cb);
     ...
     * @param forParent Parent view to monitor
     */
    private MyViewDragHelper(@NonNull Context context, @NonNull ViewGroup forParent,
                             @NonNull MyViewDragHelper.Callback cb) {
        if (forParent == null) {
     ...
     * The callback's {@link androidx.customview.widget.ViewDragHelper.Callback#onEdgeTouched(int, int)} and
     * {@link androidx.customview.widget.ViewDragHelper.Callback#onEdgeDragStarted(int, int)} methods will only be invoked
     ...
     * but {@link androidx.customview.widget.ViewDragHelper.Callback#tryCaptureView(android.view.View, int)} will not be asked permission to
     * capture this view.
     *
     ...
     */
    public boolean smoothSlideViewTo(@NonNull View child, int finalLeft, int finalTop,boolean needMaxLimit,int newSettleDuration) {
        mCapturedView = child;
        mActivePointerId = INVALID_POINTER;

        boolean continueSliding = forceSettleCapturedViewAt(finalLeft, finalTop, 0, 0, needMaxLimit, newSettleDuration);
     ...
     * @return true if animation should continue through {@link #continueSettling(boolean)} calls
     */
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel, Boolean needMaxLimit, int newSettleDuration) {
        ...
        final int duration = computeSettleDuration(mCapturedView, dx, dy, xvel, yvel, needMaxLimit, newSettleDuration);
    ...
    private int computeSettleDuration(View child, int dx, int dy, int xvel, int yvel, Boolean needMaxLimit, int newSettleDuration) {
        ...
        int xduration = computeAxisDuration(dx, xvel, mCallback.getViewHorizontalDragRange(child), true, 0);
        int yduration = computeAxisDuration(dy, yvel, mCallback.getViewVerticalDragRange(child), needMaxLimit, newSettleDuration);

        return (int) (xduration * xweight + yduration * yweight);
    }

    private int computeAxisDuration(int delta, int velocity, int motionRange, Boolean needMaxLimit, int newSettleDuration) {
        if (delta == 0) {
            return 0;
        }
        ...
        if (velocity > 0) {
            ...
            if (needMaxLimit)
                ...
                duration = (int) ((range + 1) * newSettleDuration);
        }
        if (needMaxLimit)
